            ...
            <div class="row product-images clear">
                ...
                <div class="all-product-items col-md-9 col-xs-12 mb"><div id="ps-loading-bk" style=" display: none;"></div>
                    ...
                    if(!isset($_GET['cat']) || empty($_GET['cat']) || intval($_GET['cat']==0)) {
                        $q3 = $conn->prepare("SELECT * FROM products WHERE product_type='merchant' AND approve=1 ORDER BY id desc");
                    }
                    else{
                        $cid =secureTxt($_GET['cat']);
                        $q3 = $conn->prepare("SELECT products.id,products.name,products.points,products.thumbnail,products.price,products.details ,product_cats.product_cat_title FROM products LEFT JOIN product_cats ON products.product_cat_id=product_cats.id WHERE product_type='merchant' AND product_cat_id=:cid  AND approve = 1 ORDER BY id desc");
