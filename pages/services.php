            ...
            <div class="row product-images clear">
                    ...
                    <div class="col-md-9 col-xs-4">
                        ...
                        <ul class="list-unstyled">
                            ...
                            while($read = $q3->fetch(PDO::FETCH_OBJ)){
                                ...
                                ?>
                                <li style="font-family: 'LLBrownWeb-Regular',Verdana,sans-serif"><a style="<?php if(isset($_GET['cat']) AND $_GET['cat']== $read->id)echo 'color:#009426;font-weight:bold'?>" href="services?cat=<?php echo $read->id?>" data-id="1" data-href="/local-deals?cat=1"><?php echo $read->service_cat_title?></a></li>
                            <?php } ?>
                        </ul>
                ...
                <div class="module row"><div id="ps-loading-bk" style=" display: none;"></div>
                    ...
                    if(!isset($_GET['cat']) || empty($_GET['cat']) || intval($_GET['cat']==0)) {
                        $q3 = $conn->prepare("SELECT * FROM services WHERE service_type='seller' AND approve=1 ORDER BY id desc");
                    }
                    else{
                        $cid =secureTxt($_GET['cat']);
                        $q3 = $conn->prepare("SELECT services.id,services.name,services.points,services.thumbnail,services.price,services.details ,service_cats.service_cat_title FROM services LEFT JOIN service_cats ON services.service_cat_id=service_cats.id WHERE service_type='seller' AND service_cat_id=:cid  AND approve = 1 ORDER BY id desc");
