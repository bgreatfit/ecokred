            ...
            <div class="row product-images clear">
                ...
                <div class="module row"><div id="ps-loading-bk" style=" display: none;"></div>
                    ...
                    if(!isset($_GET['cat']) || empty($_GET['cat']) || intval($_GET['cat']==0)) {
                        $q3 = $conn->prepare("SELECT * FROM services WHERE service_type='seller' AND approve=1");
                    }
                    else{
                        $cid =secureTxt($_GET['cat']);
                        $q3 = $conn->prepare("SELECT services.id,services.name,services.points,services.thumbnail,services.price,services.details ,service_cats.service_cat_title FROM services LEFT JOIN service_cats ON services.service_cat_id=service_cats.id WHERE service_type='seller' AND service_cat_id=:cid  AND approve = 1");
